...
const aas0 = async (bot, chat_id) => {
  await bot.sendMessage(chat_id, "Rekmala bo'limi", {
    reply_markup: {resize_keyboard: true, keyboard: keyboard.super_admin.advertising}
  })
}
...
const superAdminAdvertising = async (bot, chat_id, text) => {
  ...
  if (text === kb.super_admin.pages.advertising) await aas0(bot, chat_id)

  if (text === kb.super_admin.advertising.number) await aas1(bot, chat_id)

  if (text === kb.super_admin.advertising.all) await aas2(bot, chat_id)

  if (text === kb.super_admin.advertising.add) await aas3(bot, chat_id)
...
module.exports = {superAdminAdvertising, aas8}
